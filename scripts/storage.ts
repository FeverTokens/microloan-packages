 ...
 */
export function computeStorageSlot(
  baseString: string,
  verbose = false,
): string {
  if (verbose) {
  ...
  if (verbose) {
    console.log(
      "Step 2 - Base hash minus one:",
      `0x${baseHashMinusOne.toString(16)}`,
    );
  }
  ...
  if (verbose) {
    console.log(
      "Step 5 - Inverted 0xff bytes32:",
      `0x${invertedFF.toString(16).padStart(64, "0")}`,
    );
  }
 ...
 */
export function computeAllStorageSlots(
  verbose = false,
  ...
  Object.entries(STORAGE_SLOTS).forEach(([name, baseString]) => {
    if (verbose) {
      console.log(`\n--- Computing ${name} ---`);
    }
